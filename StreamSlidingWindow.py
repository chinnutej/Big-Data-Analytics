...
from ​pyspark.streaming ​import ​StreamingContext from ​pyspark.sql ​import ​SQLContext
import ​json
import ​re
import ​Algorithmia
...
TagCount = namedtuple(​'TagCount'​, (​"VulText"​,​"VulCount"​,​"OpText"​,​"OpCount"​)) def ​clean_tweet(text):
...
filtered_sentence.append(w) ​return ​filtered_sentence
def ​extractTweetText(tweetJSON): ​if ​(​'text' ​in ​tweetJSON):
jsonTweetsText =clean_tweet(tweetJSON[​'text'​]) ​#tweetsText = removeStopWords(jsonTweetsText)
 ​return ​jsonTweetsText ​else​:
​return ​''
def ​ratioOfTweetsContainingOpinion(text): ​global ​opinionCount
​global ​totalCount
text_tweet = [textQ ​for ​textQ ​in ​text.split(​" "​)] ​#print(text_tweet)
​totalCount = ​1
...
​return ​(​"Positive"​,​1​) ​else​:
​return ​(​"Positive"​,​0​)
def ​get_tweet_sentiment_Neu(text):
analysis = TextBlob(text)
​# set sentiment
​if ​analysis.sentiment.polarity == ​0​:
​return ​(​"Neutral"​,​1​) ​else​:
​return ​(​"Neutral"​,​0​)
...
tweetRDD = tweetsDStream.map(​lambda ​tweet: extractTweetText(json.loads(tweet))) words = tweetRDD.map(​lambda
text:[ratioOfTweetsContainingTentative(text),presenceOfVulgarity(text),ratioOfTweetsContainingOpinion(text),get_twe et_sentiment_Pos(text),get_tweet_sentiment_Neu(text),get_tweet_sentiment_Neg(text)])
countWords= words.map(​lambda ​x : list(chain.from_iterable(x)))
countWords.map(​lambda ​x : (x[​1​],x[​3​],x[​5​],x[​6​],x[​8​],x[​10​],x[​12​])).saveAsTextFiles(path+​"/text.txt"​) ​#col.union()map(lambda x : ).saveAsTextFiles("/Users/abhaymone/Documents/text.txt") #countWords.saveAsTextFiles()
...
# time.sleep(15)
# count = count + 1
# textTwitter = sqlContext.sql('select SUM(VulCount),SUM(OpCount) from TempTable group by VulText,
VulCount')
# for row in textTwitter.collect():
# print(row['SUM(VulCount)'], row['SUM(OpCount)']) ​ssc.awaitTermination(batchInterval * ​5 ​* ​1000​)
